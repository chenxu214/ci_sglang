    ...
    *,
    ...
    if cu_seqlens is not None and cu_seqlens.shape[0] > 2:
        ...
        seq_starts = torch.tensor(
            [0],
            dtype=torch.long,
            device=cu_seqlens.device if cu_seqlens is not None else "cpu",
        )
    ...
    def decode(
        ...
        **kwargs,
        ...
        if _KDA_USE_TORCH_NATIVE:
            if _diag:
                s_norm = (
                    ssm_states[cache_indices[0]].float().norm().item()
                    if cache_indices[0] >= 0
                    else -1
                )
                print(
                    f"[KDA-torch decode] q_norm={q.float().norm().item():.2f} "
            ...
            if _diag and cache_indices[0] >= 0:
                ...
                print(
                    f"[KDA-torch decode] out_norm={out.float().norm().item():.2f} "
        ...
        if _diag:
            ...
            if not hasattr(self, "_decode_shape_logged"):
                ...
                print(
                    f"[KDA-decode-shape] q={list(q_bak.shape)} k={list(k_bak.shape)} v={list(v_bak.shape)} "
                ...
                print(
                    f"[KDA-decode-shape] q_norm={q_bak.float().norm().item():.4f} k_norm={k_bak.float().norm().item():.4f} "
            ...
            ssm_t = (
                ssm_states[cache_indices[0]].float() if cache_indices[0] >= 0 else None
            )
            ssm_n = (
                ssm_native[cache_indices[0]].float() if cache_indices[0] >= 0 else None
            )
            ...
            print(
                f"[KDA-decode-cmp] triton_norm={t_f.norm().item():.4f} native_norm={n_f.norm().item():.4f} "
    ...
    def extend(
        ...
        **kwargs,
        ...
        if _os_early.getenv("SGLANG_KDA_DEBUG", "0") == "1" and A_log is not None:
            ...
            print(
                f"[KDA-extend-input] q_nan={torch.isnan(q).any().item()} "
        ...
        if _KDA_USE_TORCH_NATIVE_EXTEND and A_log is not None:
            ...
            if _os.getenv("SGLANG_KDA_DEBUG", "0") == "1":
                ...
                print(
                    f"[KDA-torch extend] q={list(q.shape)} k={list(k.shape)} v={list(v.shape)} "
        ...
        if _kda_debug and A_log is not None:
            ...
            gate_x = g_f + dt_bias_f.view(1, 1, -1, g_f.shape[-1])
            gate_act = -torch.exp(A_log_f.view(1, 1, -1, 1)) * F.softplus(
                gate_x, beta=1.0, threshold=20.0
            )
            gate_exp = torch.exp(gate_act)
            print(
                f"[KDA-extend-cmp] triton_norm={out_triton.float().norm().item():.4f} "
                ...
                f"q_norm={q.float().norm().item():.4f} "
                f"g_min={g_f.min().item():.4f} g_max={g_f.max().item():.4f} "
