...
class FusedMoE(torch.nn.Module):
    ...
    def offload_expert_weights_to_dram(self):
        ...
        # register_layer_batch() creates temporary CPU tensors via .cpu() for
    ...
    def start_prefill_prefetch(self):
        ...
        Call wait_prefill_prefetch() before compute and free_prefill_cache()
        after compute.
        """
        if (
    ...
    def free_prefill_cache(self):
        ...
        N>0: frees _prefetched_buffers (allocated by start_prefill_prefetch).
        N=0: frees _shared_hbm_buffers (allocated by _load_experts_on_demand)
        to avoid HBM accumulation across layers.
        """
        if (
            not self._dram_offload_enabled
            or self._expert_weight_store is None
