...
class NPUCompressedTensorsW4A16mxfp4MoE(CompressedTensorsMoEScheme):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if not _skip_nz_cast:
            # Non-offload: full NZ + int4pack format cast
            ...
            # DRAM offload: unpack + transpose only, skip NZ/int4pack
            # (NZ format cannot be CPU-copied). Apply at forward time
            # in w4a16_mxfp4_gmm_npu.
            layer.w13_weight.data = unpack_uint8_to_fp4_return_float32(layer.w13_weight.data)
...
def w4a16_mxfp4_gmm_npu(
    ...
    # DRAM offload: weights stored in ND float32 format (unpack + transpose,
    # no NZ/int4pack). Convert to NZ + int4pack at forward time so the
    # antiquant_scale API can consume them. Non-offload weights are already
    # in int4pack format (from process_weights_after_loading), skip.
    if weight.dtype == torch.float32:
        weight = torch_npu.npu_format_cast(weight, 29, customize_dtype=torch.bfloat16)
        weight = torch_npu.npu_convert_weight_to_int4pack(weight).contiguous()
