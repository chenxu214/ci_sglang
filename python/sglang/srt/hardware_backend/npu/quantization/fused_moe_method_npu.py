...
class NPUW4A8MxFpDynamicMoEMethod(_NPUFusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        w13_weight_scale = (
            ...
            .reshape(
                w13_weight_scale.shape[0],
                w13_weight_scale.shape[2],
                w13_weight_scale.shape[1] // 2,
                2,
            )
            .transpose(1, 2)
        )
        w2_weight_scale = (
            ...
            .reshape(
                w2_weight_scale.shape[0],
                w2_weight_scale.shape[2],
                w2_weight_scale.shape[1] // 2,
                2,
            )
            .transpose(1, 2)
        )
    ...
    def apply(
        ...
        if not torch.npu.is_current_stream_capturing():
            ...
            output = npu_fused_experts_w4a8_mxfp_decode(
                hidden_states=x,
                w13=layer.w13_weight,
                w13_weight_scale_inv=layer.w13_weight_scale,
                w2=layer.w2_weight,
    ...
    def apply_without_routing_weights(
        ...
        torch.save(hidden_states,
                   f"/home/chenxu/plog/modelslim_before_gmm1_{get_count()}_{torch.distributed.get_rank()}.pt")
        hidden_states = w4a8_mxfp_gmm_npu(
            input=hidden_states,
        ...
        torch.save(hidden_states,
                   f"/home/chenxu/plog/modelslim_gmm1_{get_count()}_{torch.distributed.get_rank()}.pt")
        ...
        torch.save(hidden_states,
                   f"/home/chenxu/plog/modelslim_gmm2_{get_count()}_{torch.distributed.get_rank()}.pt")
        return hidden_states
